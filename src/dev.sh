...
# Install .NET SDK
# On linux-s390x, there is no support for dotnet-install.sh, so we must rely
# on a pre-installed dotnet-sdk-8.0 (Canonical ships it as an apt package on
# Ubuntu 24.04 s390x, so `sudo apt install dotnet-sdk-8.0` is the recipe).
